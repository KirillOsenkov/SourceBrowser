...
namespace Microsoft.SourceBrowser.HtmlGenerator
{
    public class Program
    {
        ...
        private static void AddProject(List<string> projects, string path)
        {
            ...
            {
                FileError fileStatus = FileUtilities.GetFileStatus(path);
                if (fileStatus == FileError.AccessDenied)
                {
                    Log.Exception("Input file is locked or no permissions to access it: " + path, isSevere: false);
                }
                else if (fileStatus == FileError.NotFound)
                {
                    Log.Exception("Project not found: " + path, isSevere: false);
                }
            }
            else if (IsSupportedProject(project))
            {
